...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!uc || !lang) return {};
  return {
    title: `Live Translator for ${uc.label} in ${lang.language}`,
    description: `Captio AI provides real-time ${lang.language} translation for deaf and hard of hearing people during ${uc.label.toLowerCase()}. iPhone app.`,
  };
}

export default async function LiveTranslatorLanguagePage({ params }: Props) {
  const { useCase, language } = await params;
  const uc = getUseCaseBySlug(useCase);
  const lang = getLanguageBySlug(language);
  if (!uc || !lang) notFound();
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-16">
      <h1 className="text-3xl font-bold mb-4">
        Live Translator for {uc.label} in {lang.language}
      </h1>
