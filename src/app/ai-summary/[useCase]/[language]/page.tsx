...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!uc || !lang) return {};
  return {
    title: `AI Summary for ${uc.label} in ${lang.language}`,
    description: `Captio AI Summary creates structured ${lang.language} summaries from ${uc.label.toLowerCase()} transcripts for deaf and hard of hearing people.`,
  };
}

export default async function AiSummaryLanguagePage({ params }: Props) {
  const { useCase, language } = await params;
  const uc = getUseCaseBySlug(useCase);
  const lang = getLanguageBySlug(language);
  if (!uc || !lang) notFound();
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-16">
      <h1 className="text-3xl font-bold mb-4">
        AI Summary for {uc.label} in {lang.language}
      </h1>
